...
const finder = (config: string | FinderConfig = { paths: ['.'] }) => {
    const returnable = {
        length: 0,
        types: [],
        names: [],
...
try{
    ...
    const getFiles = (dir:any) => {
        ...
        let dirStat:Stats = lstatSync(dir)
        let direntsArray:Dirent[] = []

        if(dirStat.isDirectory()){
            log.getFiles('Path is directory:', dir)
            direntsArray = readdirSync(dir, { withFileTypes: true });
        }else{
        ...
        const files:any[] = direntsArray.map((dirent: Dirent | string) => {
            ...
            const resolvedPath:string = resolve(dir === dirent ? '' : dir, path);
